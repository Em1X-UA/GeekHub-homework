...
class User:
    ...
    def user_menu(self):
        """
        Main menu, redirects to other functions by user choice
        """
        ...
        match user_choice:
            ...
            case 5:
                return self.check_currency_rates()
            case 0:
    ...
    def check_balance(self):
        """
        Get user balance from ATM db and print it
        """
    ...
    def withdraw_money(self):
        """
        Get amount from user, check if possible to withdraw if possible -
        withdraw from users and ATM balance by included functions
        """
    ...
    def top_up_balance(self):
        """
        Get amount from user, check it and top up his balance if ok
        """
    ...
    def check_logs(self):
        """
        Printing all operations with funds for current user.
        """
    ...
    def back_main_menu(self):
        """
        Backing to main menu after operations
        """
    ...
    def check_currency_rates(self):
        """
        Get currency rates from ATM module and prints it.
        """

        curr = 'UAH'
        data = Atm.get_currency_rates()
        print(f'Current rates for {curr}:\n')
        print('Currency: Buy rate | Sale rate')

        for val, rates in data.items():
            print(f'{val}: {rates[0]} {curr} | {rates[1]} {curr}')
        self.back_main_menu()
