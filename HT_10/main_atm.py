...
“вилучені” з банкомату. Тобто якщо до операції в банкоматі
...
При запускі
Вхід - Реєстрація (з перевіркою валідності/складності введених даних) - Вихід
Для користувача
Баланс - Поповнення - Зняття - Історія транзакцій - Вихід на стартове меню
Для інкасатора
Наявні купюри/баланс тощо - Зміна кількості купюр - Повна історія операцій по
банкомату (дії всіх користувачів та інкасаторів) - Вихід на стартове меню
- обов’язкове дотримання РЕР8
(якщо самостійно ніяк, то https://flake8.pycqa.org/en/latest/ вам в допомогу)
...
def check_all_logs(user):
    clear()
    print(f'---{user} transactions history---')
    with sqlite3.connect(atm_data) as db:
        cursor = db.cursor()
        cursor.execute("SELECT * FROM transaction_logs")
        for id_op, user_log, date, operation, balance_changes in cursor.fetchall():
            print(f'ID: {id_op}')
...
def withdraw_money(user):
    ...
    min_banknote = min(available_banknotes())
    clear()
    ...
    for _ in range(input_attempts):
        ...
        try:
            ...
            if user_input > balance:
                ...
                banknotes = count_cash(user_input)
